...
begin
if(a<>nil)then begin
 EdadTotal:= a^.dato.edad - 1 + EdadTotal(a^.HI)+ EdadTotal(a^.HD) 
...
begin
CalcularPromedio:= EdadTotal(a)/CantidadTotal(a);

end;
{ ix. Informe, a partir de dos valores que se leen, la cantidad de socios en el arbol cuyo numero de socio se encuentra entre 
        los dos valores ingresados. Debe invocar a un modulo recursivo que reciba los dos valores leÃ­dos y retorne dicha cantidad. }
        
procedure InformarValoresEnArea(min,max:integer; a:arbol;var cant:integer);
begin
if(a<>nil)then begin
if((a^.dato.numero<=max) and (a^.dato.numero >= min)) then begin 
cant:= cant + 1;
InformarValoresEnArea(min,max,a^.HI,cant);
InformarValoresEnArea(min,max,a^.HD,cant);

end else if(a^.dato.numero < min) then InformarValoresEnArea(min,max,a^.HD,cant) 
else if(a^.dato.numero > max) then InformarValoresEnArea(min,max,a^.HI,cant);
...
//. Informe los numeros de socio en orden creciente. 
...
procedure  InformarNumerosSociosOrdenMasMenos(a:arbol);
begin
if(a <> nil)then begin
		 InformarNumerosSociosOrdenMasMenos(a^.HD);
		writeln(a^.dato.numero);
		  InformarNumerosSociosOrdenMasMenos(a^.HI);
end;
end;
var a: arbol;
     cant:integer;
Begin
  ...
  writeln('cantidad total de socios: ',CantidadTotal(a));
   writeln('Promedio de EDADES: ');
   write(CalcularPromedio(a):1:2);
  
    InformarValoresEnArea (10, 25,a, cant);
    
    InformarNumerosSociosOrdenCreciente (a); 
    InformarNumerosSociosOrdenMasMenos (a)
}
  readln;
End.
